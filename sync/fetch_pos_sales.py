"""
fetch_pos_sales.py — Full load from Fabric → DuckDB for POS / AIL Sales.

Strategy: TRUNCATE + LOAD every run.
The date window (current month, or last month on day-1) is computed by SQL
Server itself via GETDATE() — no Python date args needed.

Usage (run from the SEMANTIC-LAYER folder):
    python sync/fetch_pos_sales.py
...
# ── Query ──────────────────────────────────────────────────────────────────
# Date window is resolved by SQL Server (GETDATE()), not Python.
...
def fetch_all() -> pd.DataFrame:
    ...
    log.info("Fetching POS sales (current month window, resolved by SQL Server) …")
...
def delete_and_insert(df: pd.DataFrame) -> None:
    """Delete rows by INVOICENO then insert fresh data (preserves history)."""
    ...
    # Delete only the invoice numbers present in this fetch
    invoice_nos = df['INVOICENO'].dropna().unique().tolist()
    log.info(f"Deleting {len(invoice_nos):,} invoice numbers from DuckDB …")
    con.execute("""
